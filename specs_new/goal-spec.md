...
## 1. Problem Statement
<!-- What problem does this app solve? Who has this problem today, and how do they cope without this tool? -->
...
## 2. Target Users
<!-- Who uses this app? e.g. solo travelers, families, groups of friends. Include any relevant personas. -->
...
## 3. Core Use Cases
<!-- The primary user journeys, as short scenarios. e.g. "As a user, I want to create a trip and add destinations so that I can see my itinerary in one place." -->
- As a user, I want to create a trip and add destinations so that I can see my itinerary in one place.
- As a user, I want to describe a trip in English or Hinglish, review the extracted destination/dates/trip type, and confirm its creation in chat, with manual entry available as an alternative (`AI-spec.md`).
- For each destination, I want to enter the start and end date of the trip, and plan my activities in a day-by-day manner
- After the plan is finalized, I want to print out the itinerary in a PDF

## 4. Core Features (MVP)
<!-- Bullet list of must-have features for v1. Be specific enough to scope work, not exhaustive. -->
...
- Approved AI enhancement: conversational collection of those same trip fields, with English replies, explicit confirmation, and browser-local restoration of unfinished drafts until creation or logout. Detailed behavior is owned by `AI-spec.md`; UI is owned by `frontend-spec.md`.
...
<!-- Features explicitly deferred. Helps prevent scope creep. -->
- User authentication workflow with email

## 6. Out of Scope
<!-- Explicitly excluded from this project, to avoid ambiguity later. -->
- Auto populate day-by-day plan through a drop down
- Auto populate suggestions on the basis of trip type

## 7. Success Metrics
<!-- How do we know this app is working? e.g. adoption, task completion, performance targets. -->
...
<!-- Budget, timeline, team size, must-use tech, offline requirements, target platforms (web/mobile), etc. -->
- Timeline: 2 weekends
- Team Size: 1 person
- Target Platform: Web only, built responsively so it also works on mobile browsers (no native mobile app)

## 9. Glossary
<!-- Domain terms specific to this app (e.g. "Trip", "Leg", "Itinerary item") and their precise meaning, to keep frontend/backend/API specs consistent. -->
| Term | Definition |
|------|------------|
